  ...
  try {
    ...
    if(payload.uploadedImages && payload.uploadedImages.length > 0){
      for (let index = 0; index < payload.uploadedImages.length; index++) {
        ...
        let createdImageMapping = await EventImageMapping.create({imageName:imageName,imagePath:imagePath,eventScheduleId:createdEvent.id});
      }
    }
    if(payload.usedItems && payload.usedItems.length > 0){
      for (let index = 0; index < payload.usedItems.length; index++) {
        ...
        const existingItem = await DecorationItem.findOne({where:{id:usedItem.id}});
        const updatedItem = {quantity:existingItem.quantity-usedItem.usedQuantity};
        let updatedProduct = await DecorationItem.update(updatedItem,{where:{id:usedItem.id}})
        let createdItemMapping = await EventItemMapping.create({usedQuantity:usedItem.usedQuantity,decorationItemId:usedItem.id,eventScheduleId:createdEvent.id});
      }
    }
    let newEvent = await EventSchedule.findOne({where:{id:createdEvent.id},include:[EventImageMapping,EventItemMapping]})
    return newEvent;
  } catch (error) {
    if(createdEvent){
      let deletedItemMapping = await EventItemMapping.destroy({where:{eventScheduleId:createdEvent.Id}});
      let deletedImageMapping = await EventImageMapping.destroy({where:{eventScheduleId:createdEvent.Id}});
      let deletedEvent = await EventSchedule.destroy({where:{id:createdEvent.Id}});
    }
  ...
  try {
      ...
      if(payload.newUploadedImages && payload.newUploadedImages.length > 0){
        for (let index = 0; index < payload.newUploadedImages.length; index++) {
          ...
          const imagePath = "/uploads/"+imageName;
          let createdImageMapping = await EventImageMapping.create({imageName:imageName,imagePath:imagePath,eventScheduleId:payload.id});
        }
      }
      if(payload.newUsedItemList && payload.newUsedItemList.length > 0){
        for (let index = 0; index < payload.newUsedItemList.length; index++) {
          ...
          const updatedItem = {quantity:existingItem.quantity-usedItem.usedQuantity};
          let updatedProduct = await DecorationItem.update(updatedItem,{where:{id:usedItem.id}})
          let createdItemMapping = await EventItemMapping.create({usedQuantity:usedItem.usedQuantity,decorationItemId:usedItem.id,eventScheduleId:payload.id});
        }
      }
      let newEvent = await EventSchedule.findOne({where:{id:createdEvent.id},include:[EventImageMapping,EventItemMapping]})
      return newEvent;
...
exports.getAllEventSchedule = async (req, res, next) => {
  try {
    ...
    let response = await EventSchedule.findAll({
      where: query,
      include:[{model:Community},{model:EventImageMapping},{model:EventItemMapping,include:DecorationItem}],
      order: [[EventSchedule.sequelize.fn('STR_TO_DATE', EventSchedule.sequelize.col('scheduledStartDate'), '%m-%d-%Y'), 'ASC']]
    });
    let eventList = [];
    if(response && response.length>0){
      for (let index = 0; index < response.length; index++) {
        ...
        if(event.eventImageMappings && event.eventImageMappings.length>0){
          for (let index = 0; index < event.eventImageMappings.length; index++) {
        ...
        if(event.eventItemMappings  && event.eventItemMappings .length>0){
          for (let index = 0; index < event.eventItemMappings .length; index++) {
            ...
            itemList.push({itemName:itemMap.decorationItem.itemName,usedQuantity:itemMap.usedQuantity,unitType:itemMap.decorationItem.unitType});
          }
        }
...
exports.deleteEventSchedule = async (req, res, next) => {
  try {
    ...
    await db.sequelize.transaction(async (t) => {
      let eventImages = await EventImageMapping.findAll({ where: { eventScheduleId: req.query.id }, transaction: t });
      await EventItemMapping.destroy({ where: { eventScheduleId: req.query.id }, transaction: t });
      await EventImageMapping.destroy({ where: { eventScheduleId: req.query.id }, transaction: t });
      await EventSchedule.destroy({ where: { id: req.query.id }, transaction: t });
